module character(
    ...
    output reg [2:0] state,                             // state indicator according to localparam below
    output reg [6:0] frame_counter,                     // frame tick counter output
    ...
    always @(*) begin:COMB
        ...
        if (special_hit_flag && state != s_move_backward) begin 
            KO_flag = 1;
            // NOTE THAT, I assigned next_state as idle, we can arrange it later.
            next_state = s_idle;
        end 
    ...
    always @(posedge game_clk || negedge rst) begin:SEQ
        if(rst) begin 
            state <= s_idle;
            frame_counter <= 0;
